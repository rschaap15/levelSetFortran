...
!*************************************************************************************!
! Describe Convective Motion
!*************************************************************************************!

! describe the motion you wish the zero level set to go under here
...
DO n = 1,iter
   ...
   ! real time for time integration
   rTime = (n-1)*h1
   ...
   ! Convect the zero level set
   DO i = 0,nx
      DO j = 0,ny
         DO k = 0,nz
            IF (phiNB(i,j,k) == 1) THEN
               IF (Fx > 0) THEN
                  phiX = gradPhi(i,j,k,1,1) 
               ELSE 
                  phiX = gradPhi(i,j,k,1,2)
               END IF
               IF (Fy > 0) THEN
                  phiY = gradPhi(i,j,k,2,1) 
               ELSE 
                  phiY = gradPhi(i,j,k,2,2)
               END IF
               IF (Fz > 0) THEN
                  phiZ = gradPhi(i,j,k,3,1) 
               ELSE 
                  phiZ = gradPhi(i,j,k,3,2)
               END IF
               gM = gradPhiMag(i,j,k) 
               CALL minMax(i,j,k,nx,ny,nz,dx,phi,grad2Phi,gradMixPhi,F,gridX)
               k1 = -(Fx*phiX + Fy*phiY + Fz*phiZ) !+F
               phi1(i,j,k) = phi(i,j,k) + h1*k1
            END IF
   ...
   DO i = 0,nx
      DO j = 0,ny
         DO k = 0,nz
            IF (phiSB(i,j,k) == 1) THEN
               CALL weno(gM,i,j,k,nx,ny,nz,dx,phi1,gradPhi,gradPhiMag)
            END IF
         END DO
      END DO
   END DO
   ...
   DO i = 0,nx
      DO j = 0,ny
         DO k = 0,nz
            IF (phiSB(i,j,k) == 1) THEN
               CALL weno(gM,i,j,k,nx,ny,nz,dx,phi2,gradPhi,gradPhiMag)
            END IF
         END DO
      END DO
   END DO
   ...
   phiErr = sqrt(phiErr/(nx*ny*nz))
   IF (phiErr < 1.E-7) THEN
      PRINT*, " Time integration has finished marching "
      EXIT
   END IF
   ...
   PRINT*, " Iteration: ",n," ", " RMS Error: ",phiErr," "," Real Time: ",rTime
   ...
   CFL = 0.01
   h = CFL*dxx
   CALL reinit(phi,gradPhi,gradPhiMag,nx,ny,nz,reIter,dx,h)
